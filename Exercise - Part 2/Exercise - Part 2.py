...
def count_a_number(numbers, number): #return numbers.count(number) wäre eingebaute Funktion
    counter = 0
    for e in numbers:
        if number == e:
            counter += 1
...
def play_with_lists(numbers, number):
    ...
    replaced_numbers2 = [1 if x == number else x for x in
                         replaced_numbers]  # Task 2 mit List Comprehension, hier werden alle 8er ersetzt
